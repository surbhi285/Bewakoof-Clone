...
//   console.log(singleOrder.data.items);
    ...
    return (
      ...
      <Flex>
       ...
       {singleOrder?.data?.items.map((order)=>(
        <Box>
        <div key={order._id} style={{marginTop:"50px", flex:1}}>
        <Flex style={{gap:"15%", color: "rgba(0,0,0,.7)", fontSize:"14px"}}>
        <h5>order# <span style={{fontWeight:"bold", color:"black"}}>{order?.product._id}</span></h5>
        <h5>ORDER PLACED<span style={{fontWeight:"bold", marginLeft:"10px"}}>{order?.product.createdAt && order?.product.createdAt.split('.')[0]}</span></h5>
        </Flex>
        ...
        <Button style={{borderRadius:"50px", width:"40px", height:"40px", border:"1px solid #2ca003", backgroundColor:"#e7ffeb", marginLeft:"50px"}}><TiTick style={{color:"#2ca003"}}/></Button>
        <Text style={{color:"#2ca003", marginTop:"5px", paddingLeft:"5px"}}>CONFIRMED</Text>
        </Flex>
        <Text style={{marginTop:"-2px"}}>______________________</Text>
        <Flex style={{flexDirection:"column"}}>
        <Button style={{borderRadius:"50px", width:"40px", height:"40px", border:"1px solid #2ca003", backgroundColor:"#e7ffeb"}}><TiTick style={{color:"#2ca003"}}/></Button>
        ...
        <Button style={{height:"20px", color:"#fe8f00", backgroundColor:"#fff9f2", border:"1px solid #fe8f00", width:"40px"}}>
        {order?.shipmentDetails?.type}
        </Button>
        <Text> {order?.shipmentDetails?.address?.street}, {order?.shipmentDetails?.address?.city}</Text>
        </Box>
        </div>
        ...
        <Text>{order?.shipmentDetails?.address?.street}, {order?.shipmentDetails?.address?.city}</Text>
         ...
         <Flex style={{justifyContent:"space-between",marginTop:"-15px", borderBottom:"2px solid #d6d6d6", marginLeft:"20px", marginRight:"20px"}}>
